...
void test_error(std::vector<float>& rets_w,
    datastore::DataStore<LabeledPointHObj<float, float, true>>& data_store, const std::string& error_info = "") {
...
void SGD_update(datastore::DataStore<LabeledPointHObj<float, float, true>>& data_store,
    int kv_w, int kv_u, int kv_w_old, float alpha, int num_params, int num_iters, const Info & info, int num_active_process, ml::mlworker2::PSBspWorker<float>& mlworker_u, ml::mlworker2::PSBspWorker<float>& mlworker_w, ml::mlworker2::PSBspWorker<float>& mlworker_w_old) {
    ...
    // kvworker->Wait(kv_w_old, kvworker->Pull(kv_w_old, keys, &old_rets_w, true, true, false));
    ...
    for(int i = num_iters; i > 0&&data_looper.has_next(); i--) {
        ...
        for (int j = 0; j < delta.size(); j++) {
            ...
            // record rets_w_delta
            float w_delta = (-1) * alpha * (delta[j].second - old_delta[j].second + (iter - update_counter[idx]) * rets_u[idx]);
    ...
    // num_active_process > 1, means running exp2: SGD is multiple threads, in this case, num_active_process will play a active role to calculate an avg 
...
void FGD_update(datastore::DataStore<LabeledPointHObj<float, float, true>>& data_store,
    int kv_w, int kv_u, int kv_w_old, float alpha, int num_params, const Info & info, size_t data_size, ml::mlworker2::PSBspWorker<float>& mlworker_u, ml::mlworker2::PSBspWorker<float>& mlworker_w, ml::mlworker2::PSBspWorker<float>& mlworker_w_old) {
    // auto start_time = std::chrono::steady_clock::now();
    ...
    // kvworker->Wait(kv_w_old, kvworker->Pull(kv_w_old, {}, &tmp));
	...
	if (rest_thread_process_w_size > 0 && position == tids.size() - 1) {  // rest is not 0 && current_thread is the max number
    ...
    // debug: check kv_w equals kv_w_old
    /*
    std::vector<float> test_w_old;
    kvworker->Wait(kv_w_old, kvworker->Pull(kv_w_old, keys, &test_w_old, true, true, false));
    if (info.get_cluster_id() == 0 ) {
        debug_kvstore(test_w_old, "test_w_old: ");
        debug_kvstore(rets_w, "w: ");
    }
    */
...
int main(int argc, char** argv) {
    ...
    config::InitContext(argc, argv, {"num_load_workers", "data_size",
            "svrg_exp", "svrg_fgd_workers_per_process",
            "sgd_batch_size", "sgd_continious_epoch", "sgd_stage", "num_sgd_continious_epoch", "sgd_exp3_num_threads"});
    ...
    if (svrg_exp == 1) {
       ...
       // exp3: in SGD epoch, run multiple train threads, but these multiple threads are in one process, and their worker_num_type is threads_per_cluster 
       // generate worker_num and worker_num_type based on conf
       // add fgd conf
       worker_num.push_back(svrg_fgd_workers_per_process);
       worker_num_type.push_back("threads_per_worker");
    ...
    // kv_w_old kvstore keeps the old_w to be resued in SGDs. after each FGD, kv_w_old will be updated to keep same with kv_w.
    ...
    engine.AddTask(std::move(train_task), [&kv_w, &kv_u, &kv_w_old, &data_store,  &alpha, num_params, data_size,
            &svrg_exp, &svrg_fgd_workers_per_process, &sgd_stage, &sgd_batch_size, &sgd_exp3_num_threads](const Info & info) {
        auto start_time = std::chrono::steady_clock::now();
        ...
        if (current_epoch % worker_num.size() == 0) {
            ...
            FGD_update(data_store, kv_w, kv_u, kv_w_old, alpha, num_params, info, data_size, mlworker_u, mlworker_w, mlworker_w_old); 
        } else {   // do SGD
            ...
            if (data_store_wrapper.get_data_size() == 0) {
                ...
                if (svrg_exp == 1) {
                    ...
                    SGD_update(data_store, kv_w, kv_u, kv_w_old, alpha, num_params, each_epoch_iter, info, 1, mlworker_u, mlworker_w, mlworker_w_old);
                } else if (svrg_exp == 2) {
                    if (info.get_cluster_id() == 0) {
                    ...
                    SGD_update(data_store, kv_w, kv_u, kv_w_old, alpha, num_params, 
                            each_epoch_iter, info, info.get_worker_info().get_num_processes() * svrg_fgd_workers_per_process, mlworker_u, mlworker_w, mlworker_w_old);
                } else {
                    if (info.get_cluster_id() == 0) {
                    ...
                    SGD_update(data_store, kv_w, kv_u, kv_w_old, alpha, num_params, each_epoch_iter, info, sgd_exp3_num_threads, mlworker_u, mlworker_w, mlworker_w_old);
                }
            }
        } 
        auto end_time= std::chrono::steady_clock::now();
