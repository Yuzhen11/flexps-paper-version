...
int main(int argc, char** argv) {
    // Get configs
    config::InitContext(argc, argv, {"batch_sizes", "nums_iters", "alphas", "lr_coeffs", "nums_train_workers", "num_load_workers", "report_interval", "model_input"});
    ...
    engine.AddTask(load_task, [&data_store, config, &benchmark_model, &model_file, &kv_true](const Info& info) {
        if (model_file != "" && info.get_cluster_id() == 0) {
             ...
             *     loss += lr::lr_test_objective(data, benchmark_model);
             *     count += 1;
             * }
             * loss /= count;
             * husky::LOG_I << "Benchmark model objective value: " << std::setprecision(15) << loss;
             */
        }
    ...
    engine.AddTask(train_task, [&report_interval, &data_store, config, &batch_sizes, &nums_iters, &alphas, &lr_coeffs, &nums_workers, &benchmark_model, kv_true](const Info& info) mutable {
        ...
        config.alpha = alphas[current_stage] / config.num_train_workers;
        config.learning_rate_coefficient = lr_coeffs[current_stage];
        if (info.get_cluster_id() == 0) {
            husky::LOG_I << "Stage " << current_stage << ": " << config.num_iters << "," << config.batch_size << "," << config.alpha << "," << config.learning_rate_coefficient;
        ...
        lr::sgd_train(info, data_store, config, benchmark_model, report_interval, accum_iter, false);
    });
