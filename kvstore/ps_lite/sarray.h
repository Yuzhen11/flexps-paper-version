...
class SArray {
 public:
  ...
  // serialization function into husky::base::BinStream for SArrary
  // Warning: only works for pod V
  friend husky::base::BinStream& operator<<(husky::base::BinStream& bin, const SArray<V>& sarray) {
    bin << sarray.size();
    bin.push_back_bytes(reinterpret_cast<char*>(sarray.data()), sarray.size()*sizeof(V));
    return bin;
  }
  friend husky::base::BinStream& operator>>(husky::base::BinStream& bin, SArray<V>& sarray) {
    size_t size;
    bin >> size;
