...
public:
    ...
    KVWorker(const PSInfo& info, husky::LocalMailbox& mailbox)
        : customer_(new WorkerCustomer(mailbox, [this](int kv_id, int ts, husky::base::BinStream& bin){ Process(kv_id, ts, bin); }, info.channel_id)),
          info_(info) {
        customer_->Start();
    }
    ~KVWorker() {
    ...
    template<typename Val>
    void UniqueProcess(int kv_id, int ts, husky::base::BinStream& bin) {
    ...
    void Process(int kv_id, int ts, husky::base::BinStream& bin) {
        ...
        process_map[kv_id](kv_id, ts, bin);
    }
    /*
     * Engine use this funciton to add process func
     */
    template<typename Val>
    void AddProcessFunc(int kv_id) {
        assert(process_map.find(kv_id) == process_map.end());
        process_map.insert(std::make_pair(kv_id, [this](int kv_id, int ts, husky::base::BinStream& bin){UniqueProcess<Val>(kv_id, ts, bin);}));  // push the function template in
    }
    ...
    void RunCallback(int kv_id, int ts) {
        mu_.lock();
        auto it = callbacks_[kv_id].find(ts);
        if (it != callbacks_[kv_id].end()) {
            mu_.unlock();

            it->second();

            mu_.lock();
            callbacks_[kv_id].erase(it);
        }
        mu_.unlock();
    }
private:
    ...
    // callbacks
    std::unordered_map<int, std::unordered_map<int, Callback>> callbacks_;  // {kv_id, {ts, callback}}
    // process function map
    std::unordered_map<int, std::function<void(int, int, husky::base::BinStream&)>> process_map;  // {kv_id, process()}
    std::mutex mu_;
