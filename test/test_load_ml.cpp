...
int main(int argc, char** argv) {
    ...
    engine.AddTask(task, [&data_store1](const Info& info) {
        ...
        io::LineInputFormatML infmt(2, 1001);
    ...
    auto task1 = TaskFactory::Get().CreateTask<HuskyTask>(2, 3);
    engine.AddTask(task1, [&data_store1](const Info& info) {
        // load
        auto parse_func = [](boost::string_ref& chunk) {
            if (chunk.size() == 0)
                return;
        ...
        io::LineInputFormatML infmt(3, 2001);
        infmt.set_input(husky::Context::get_param("input"));
