...
int main(int argc, char** argv) {
    ...
    engine.AddTask(task2, [](const Info& info) { husky::LOG_I << std::to_string(info.get_task_id()) + " is running"; });

    auto task3 = TaskFactory::Get().CreateTask<Task>(1, 3);
    engine.AddTask(task3, [](const Info& info) { husky::LOG_I << std::to_string(info.get_task_id()) + " is running"; });
    engine.Submit();

    // round 2
    // worker_nums: 2, 2: multiple epochs
    auto task4 = TaskFactory::Get().CreateTask<Task>(4, 2);
    engine.AddTask(task4, [](const Info& info) { husky::LOG_I << std::to_string(info.get_task_id()) + " is running"; });

    auto task5 = TaskFactory::Get().CreateTask<Task>(4, 2);
    engine.AddTask(task5, [](const Info& info) { husky::LOG_I << std::to_string(info.get_task_id()) + " is running"; });
    engine.Submit();
    ...
    engine.AddTask(task7, [](const Info& info) { husky::LOG_I << std::to_string(info.get_task_id()) + " is running"; });
    engine.Submit();
