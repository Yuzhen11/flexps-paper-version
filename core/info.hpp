...
public:
    ...
    const std::unique_ptr<ml::common::GenericMLWorker>& get_mlworker() const {
        return mlworker_;
    }
    std::unique_ptr<ml::common::GenericMLWorker>& get_mlworker() {
        return mlworker_;
    }
    Task* const get_task() const {
        return task_;
    }
    const std::unordered_map<int,int>& get_cluster_global() const {
        return cluster_id_to_global_id_;
    }
    ...
    void set_cluster_global(const std::unordered_map<int,int>& rhs) {
        cluster_id_to_global_id_ = rhs;
    }
    void set_cluster_global(std::unordered_map<int,int>&& rhs) {
        cluster_id_to_global_id_ = std::move(rhs);
    }
private:
    int local_id_;
    int global_id_;
